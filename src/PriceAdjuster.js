...
exports.PriceAdjuster = void 0;
var PriceAdjuster = /** @class */ (function () {
    function PriceAdjuster() {
        this.productArrayDate = [];
        this.todaysDate = new Date("2024-06-10");
    }
    PriceAdjuster.prototype.priceChanger = function (productData) {
        return __awaiter(this, void 0, void 0, function () {
            var x;
            return __generator(this, function (_a) {
                // date = this.todaysDate
                for (x = 0; x < productData.length; x++) {
                    if (productData[x].Category === 'Dairy') {
                        if (productData[x].Expiry_Date.getTime() < this.todaysDate.getTime() + 4) {
                            console.log(productData[x], "-50% off");
                        }
                    }
                    if (productData[x].Category === 'Canned Goods') {
                        if (productData[x].Expiry_Date.getMonth() < this.todaysDate.getMonth() + 3) {
                            console.log(productData[x], "25% Off");
                        }
                    }
                    if (productData[x].Category === 'Vegetables') {
                        if (productData[x].Expiry_Date.getTime() < this.todaysDate.getTime() + 3) {
...
exports.PriceAdjuster = PriceAdjuster;
